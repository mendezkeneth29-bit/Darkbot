...
# --- 2. BASES DE DATOS (Se reinician al apagar el bot) ---
banco_datos = {}
tienda_roles = {} 
...
class TiendaSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if role in interaction.user.roles:
            return await interaction.response.send_message("Ya tienes este rol, no seas ambicioso. 🤣", ephemeral=True)
...
class TiendaView(discord.ui.View):
    def __init__(self):
        ...
        if not options:
            options.append(discord.SelectMenuOption(label="Sin stock disponible", value="none"))
...
class GiveawayView(discord.ui.View):
    ...
    async def participar(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user in self.participantes:
            return await interaction.response.send_message("¡Ija ke dice! Ya estás dentro. 🤣", ephemeral=True)
        self.participantes.append(interaction.user)
        await interaction.response.send_message("¡Ok mañana! Te has anotado. 💜", ephemeral=True)
...
async def tienda_config(interaction: discord.Interaction, rol: discord.Role, precio: int, stock: int):
    ...
    await interaction.response.send_message(f"✅ **{rol.name}** configurado: ${precio} | Stock: {stock}. 💜", ephemeral=True)

@bot.tree.command(name="tienda", description="Ver la tienda de roles")
async def tienda(interaction: discord.Interaction):
    if not tienda_roles:
        return await interaction.response.send_message("La tienda está vacía ahora mismo. 🤣", ephemeral=True)
    
    emb = discord.Embed(title="🏪 DARKY STORE", description="Compra roles con tu dinero del chat.", color=0x010101)
    for rid, data in tienda_roles.items():
...
async def giveaway(interaction: discord.Interaction, tiempo: int, unidad: str, premio: int, canal_ganador: discord.TextChannel):
    ...
    final = int(time.time() + seg)
    view = GiveawayView(timeout=seg)
    
    emb = discord.Embed(title="🎊 SORTEO DE DINERO 🎊", description=f"💰 **Premio:** `${premio}`\n📢 **Ganador en:** {canal_ganador.mention}\n⏰ **Finaliza:** <t:{final}:R>", color=0x010101)
    ...
    if not view.participantes:
        return await canal_ganador.send(f"❌ Nadie participó por los **${premio}**. Ija ke dice... 🤣")

    ganador = random.choice(view.participantes)
    banco_datos[str(ganador.id)] = banco_datos.get(str(ganador.id), 0) + premio
    
    emb_w = discord.Embed(title="🏆 ¡GANADOR!", description=f"Felicidades {ganador.mention}, ganaste **${premio}**.\nDepositado en tu `/banco`.", color=0x010101)
    await canal_ganador.send(content=f"¡Felicidades {ganador.mention}! 🎉", embed=emb_w)

# --- 7. COMANDOS DE BANCO Y UTILIDAD ---
...
async def banco(interaction: discord.Interaction, usuario: Optional[discord.Member] = None):
    ...
    await interaction.response.send_message(embed=discord.Embed(title=f"🏦 Banco de {target.name}", description=f"Saldo: **${saldo}**", color=0x010101))
...
async def roblox(interaction: discord.Interaction, usuario: str):
    await interaction.response.defer()
    try:
        r = requests.post("https://users.roblox.com/v1/usernames/users", json={"usernames": [usuario], "excludeBannedUsers": False}).json()
        uid = r['data'][0]['id']
