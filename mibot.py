...
app = Flask(__name__)
@app.route('/')
def home(): return "Darky Bot Arsenal Completo 💜"
...
class TiendaSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not role: return await interaction.response.send_message("No encuentro el rol. 🤣", ephemeral=True)
...
class TiendaView(discord.ui.View):
    def __init__(self):
        ...
        for rid, data in tienda_roles.items():
            if data['stock'] > 0:
                options.append(discord.SelectMenuOption(label=data['nombre'], value=str(rid), description=f"${data['precio']} | Stock: {data['stock']}"))
...
class GiveawayView(discord.ui.View):
    ...
    async def participar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.participantes.append(interaction.user); await interaction.response.send_message("¡Ok mañana! Anotado. 💜", ephemeral=True)
...
@bot.tree.command(name="tienda_config", description="Setup de la tienda")
...
@bot.tree.command(name="banco", description="Tu saldo")
async def banco(interaction: discord.Interaction, usuario: Optional[discord.Member] = None):
    ...
    emb = discord.Embed(title="🏦 BANCO", description=f"Saldo de **{t.name}**: `${saldo:,}`", color=0x2b2d31)
...
async def transferir(interaction: discord.Interaction, miembro: discord.Member, cantidad: int):
    ...
    if cantidad <= 0 or banco_datos.get(aid, 0) < cantidad:
        return await interaction.response.send_message("Fallo financiero, ija. 🤣", ephemeral=True)
...
async def regalar(interaction: discord.Interaction, miembro: discord.Member, cantidad: int):
    banco_datos[str(miembro.id)] = banco_datos.get(str(miembro.id), 0) + cantidad
    await interaction.response.send_message(f"🎁 {miembro.mention} recibió **${cantidad}**. ¡Ok mañana! 💜")
...
async def roblox(interaction: discord.Interaction, usuario: str):
    ...
    try:
        ...
        emb = discord.Embed(title=f"Perfil: {usuario}", url=f"https://www.roblox.com/users/{uid}/profile", color=0x010101)
        emb.set_image(url=thumb['data'][0]['imageUrl'])
        await interaction.followup.send(embed=emb)
    except: await interaction.followup.send("No lo hallé. 🤣")
...
async def avatar(interaction: discord.Interaction, usuario: Optional[discord.Member] = None):
    t = usuario or interaction.user
    await interaction.response.send_message(embed=discord.Embed(title=f"Avatar: {t.name}", color=0x010101).set_image(url=t.display_avatar.url))

@bot.tree.command(name="userinfo", description="Info de usuario")
async def userinfo(interaction: discord.Interaction, usuario: Optional[discord.Member] = None):
    ...
    emb.add_field(name="Unido", value=f"<t:{int(t.joined_at.timestamp())}:D>", inline=True)
    emb.set_thumbnail(url=t.display_avatar.url)
    await interaction.response.send_message(embed=emb)

@bot.tree.command(name="embed", description="Mensaje embed")
