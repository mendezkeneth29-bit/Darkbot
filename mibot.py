...
# Estos no necesitan persistencia
afk_data      = {}
mensaje_count = {}
xp_cooldown   = {}
...
async def generar_playlist_img(usuario: discord.Member, canciones: list) -> discord.File:
    ...
    for n, cancion in enumerate(canciones):
        y = 78 + (n * 46)
        draw.rounded_rectangle([(34, y), (646, y + 36)], radius=8, fill=(26, 26, 26) if n % 2 == 0 else OSCU_G)
...
async def generar_logro(usuario: discord.Member, titulo: str, descripcion: str) -> discord.File:
    ...
    draw.text((tx, 48), titulo_recortado, font=fuente(20, bold=True), fill=TEXTO)
    desc_recortada = descripcion[:60] + "..." if len(descripcion) > 60 else descripcion
    draw.text((tx, 80), desc_recortada, font=fuente(13), fill=SUBTEXTO)
    nombre = usuario.display_name[:22] + "..." if len(usuario.display_name) > 22 else usuario.display_name
    draw.text((tx, 114), f"Logrado por {nombre}", font=fuente(12), fill=SUBTEXTO)
    buf = io.BytesIO()
    img.convert("RGB").save(buf, format="PNG")
    buf.seek(0)
...
async def eightball(ctx: commands.Context, *, pregunta: str):
    ...
    embed = discord.Embed(title="Pregunta Magica",
...
async def horoscopo(ctx: commands.Context, signo: str):
    ...
    if signo.lower() not in signos:
        await ctx.send("> Signo no valido.")
        return
    ...
    embed = discord.Embed(title=f"Horoscopo de {signo.capitalize()}",
...
async def roll(ctx: commands.Context):
    ...
    if miembros:
        ...
        embed   = discord.Embed(title="Usuario aleatorio",
                                description=f"> {elegido.mention} ha sido seleccionado", color=AZUL_IPOD_NUM)
        await ctx.send(embed=embed)

@bot.hybrid_command(name="timestamp", description="Convierte timestamp a fecha")
async def timestamp(ctx: commands.Context, ts: int):
    try:
        fecha = datetime.fromtimestamp(ts).strftime("%d/%m/%Y %H:%M:%S")
        embed = discord.Embed(title="Conversor de Timestamp",
                              description=f"> Timestamp: `{ts}`\n> Fecha: `{fecha}`", color=AZUL_IPOD_NUM)
        await ctx.send(embed=embed)
    except Exception:
        await ctx.send("> Timestamp invalido")
...
async def insulto(ctx: commands.Context, usuario: discord.Member = None):
    ...
    await ctx.send(f"{target.mention} {insulto_elegido}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as resp:
                ...
                embed = discord.Embed(title=post['title'],
                                      url=f"https://reddit.com{post['permalink']}", color=AZUL_IPOD_NUM)
                embed.set_image(url=post['url'])
                embed.set_footer(text=f"{post['ups']} | r/{subreddit}")
                await ctx.send(embed=embed)
    except Exception as e:
        await ctx.send(f"> Error al obtener meme: `{str(e)[:100]}`")

# =========================================================
# COMANDOS — UTILIDADES
# =========================================================

# Caracteres permitidos para calcular (solo operaciones matematicas seguras)
_CALC_ALLOWED = re.compile(r'^[\d\s\+\-\*\/\(\)\.\%\*\*]+$')

@bot.hybrid_command(name="calcular", description="Calcula una operacion matematica")
async def calcular(ctx: commands.Context, *, operacion: str):
    # Validar que solo contenga caracteres matematicos seguros
    if not _CALC_ALLOWED.match(operacion):
        embed = discord.Embed(color=0x2B55B5)
        embed.description = "> Solo se permiten operaciones matematicas basicas (+, -, *, /, **, %, parentesis)"
    ...
    try:
        ...
        embed = discord.Embed(color=0x2B55B5)
        embed.description = "> Error: Division entre cero"
        await ctx.send(embed=embed, ephemeral=True if ctx.interaction else None)
    except Exception:
        embed = discord.Embed(color=0x2B55B5)
...
async def generar_password(ctx: commands.Context, longitud: int = 16):
    import string
...
async def generar_qr(ctx: commands.Context, *, texto: str):
    ...
    embed = discord.Embed(title="Código QR Generado",
...
async def ip_info(ctx: commands.Context, direccion_ip: str):
    ...
    if len(partes) != 4 or not all(p.isdigit() and 0 <= int(p) <= 255 for p in partes):
        await ctx.send("> Formato de IP invalido")
        return
...
async def recordar(ctx: commands.Context, tiempo: str, *, mensaje: str):
    ...
    try:
        ...
        await ctx.send(embed=embed)
        return
    if unidad == 's':   segundos, texto_unidad = cantidad,         "segundo" if cantidad == 1 else "segundos"
    elif unidad == 'm': segundos, texto_unidad = cantidad * 60,    "minuto"  if cantidad == 1 else "minutos"
    elif unidad == 'h': segundos, texto_unidad = cantidad * 3600,  "hora"    if cantidad == 1 else "horas"
    elif unidad == 'd': segundos, texto_unidad = cantidad * 86400, "día"     if cantidad == 1 else "días"
    else:
    ...
    if segundos < 5:
        ...
        await ctx.send(embed=embed)
        return
    embed_confirmacion = discord.Embed(title="Recordatorio creado",
                                       description=f"Te recordaré **{mensaje}** en **{cantidad} {texto_unidad}**",
                                       color=0x2B55B5)
    await ctx.send(embed=embed_confirmacion)
    await asyncio.sleep(segundos)
    embed_recordatorio = discord.Embed(title="RECORDATORIO",
...
async def adivina_numero(ctx: commands.Context):
    ...
    for intento in range(10):
        try:
            ...
            if not mensaje.content.isdigit():
                continue
...
async def ppt_mejorado(ctx: commands.Context):
...
async def ahorcado(ctx: commands.Context):
...
async def tragamonedas(ctx: commands.Context, apuesta: int):
    ...
    if slot1 == slot2 == slot3 == "💎":
        ganancia, mensaje = apuesta * 5, "> PREMIO MAYOR! 3 DIAMANTES!"
    elif slot1 == slot2 == slot3:
        ganancia, mensaje = apuesta * 3, "> 3 IGUALES!"
    elif slot1 == slot2 or slot2 == slot3 or slot1 == slot3:
        ganancia, mensaje = apuesta * 1, "> PAR! Recuperas tu apuesta"
    else:
        ganancia, mensaje = 0, "> Nada, perdiste"
...
async def mi_puntuacion_trivia(ctx: commands.Context):
...
async def acertijo(ctx: commands.Context):
    ...
    await ctx.send(embed=embed)
    def check(m):
        return m.author == ctx.author and m.content.lower().startswith(">respuesta")
    try:
        msg               = await bot.wait_for("message", timeout=30.0, check=check)
        respuesta_usuario = msg.content.lower().replace(">respuesta", "").strip()
...
@commands.has_permissions(administrator=True)
async def crear_cupon(ctx: commands.Context, codigo: str, recompensa: int):
    gid = str(ctx.guild.id)
...
async def canjear_cupon(ctx: commands.Context, codigo: str):
    gid = str(ctx.guild.id)
...
class SpotifySearchView(discord.ui.View):
    def __init__(self, tracks: list):
        ...
        for n, track in enumerate(tracks[:4]):
            ...
            self.add_item(discord.ui.Button(label=f"{n+1}. {nombre}", url=track["url"],
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post("https://users.roblox.com/v1/usernames/users", json=data_user) as resp:
                if resp.status != 200:
                    ...
                    await (ctx.interaction.followup.send if ctx.interaction else ctx.send)(embed=embed)
                    return
...
async def reproducir(ctx: commands.Context, *, busqueda: str):
    if ctx.interaction:
        await ctx.defer()
    try:
        ...
        await (ctx.interaction.followup.send if ctx.interaction else ctx.send)(embed=embed, ephemeral=True)

@bot.hybrid_command(name="lyrics", description="Obtén la letra de una cancion")
async def lyrics(ctx: commands.Context, *, cancion: str):
    if ctx.interaction:
        await ctx.defer()
    try:
...
async def buscar_libro(ctx: commands.Context, *, query: str):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 429:
                    embed = discord.Embed(description="> Limite de busquedas alcanzado. Espera unos minutos.", color=AZUL_IPOD_NUM)
                    return await ctx.send(embed=embed)
                if response.status != 200:
                    embed = discord.Embed(description=f"> Error de conexion ({response.status})", color=AZUL_IPOD_NUM)
                    return await ctx.send(embed=embed)
                data = await response.json()
        if "items" not in data:
            embed = discord.Embed(description=f"> No encontre ningun libro para: **{query}**", color=AZUL_IPOD_NUM)
            return await ctx.send(embed=embed)
        ...
        await ctx.send(embed=discord.Embed(description="> Tiempo de espera agotado. Intenta de nuevo.", color=AZUL_IPOD_NUM))
    except Exception as e:
        await ctx.send(embed=discord.Embed(description=f"> Error inesperado: `{str(e)[:100]}`", color=AZUL_IPOD_NUM))
...
async def clima(ctx: commands.Context, *, ciudad: str):
    ...
    if ctx.interaction:
        await ctx.defer()
    url = f"http://api.openweathermap.org/data/2.5/weather?q={urllib.parse.quote(ciudad)}&appid={API_KEY}&units=metric&lang=es"
    try:
        async with aiohttp.ClientSession() as session:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status != 200:
                ...
                return
            juegos = (await resp.json())[:5]
    embed = discord.Embed(title="Juegos Gratis Recomendados", color=0x2B55B5)
    for juego in juegos:
        embed.add_field(name=juego.get('title', 'Sin título'),
                        value=f"{juego.get('genre','Desconocido')} | {juego.get('platform','PC')}\n[Descargar]({juego.get('game_url','')})",
                        inline=False)
...
async def pelicula(ctx: commands.Context, *, nombre: str):
    ...
    if not API_KEY:
        ...
        return
    url_buscar = f"https://api.themoviedb.org/3/search/movie?api_key={API_KEY}&query={urllib.parse.quote(nombre)}&language=es"
    async with aiohttp.ClientSession() as session:
        async with session.get(url_buscar) as resp:
            ...
            if not resultados:
                ...
                return
            peli = resultados[0]
        url_detalles = f"https://api.themoviedb.org/3/movie/{peli.get('id')}?api_key={API_KEY}&language=es"
...
async def pokemon(ctx: commands.Context, *, nombre: str):
    ...
    url    = f"https://pokeapi.co/api/v2/pokemon/{urllib.parse.quote(nombre)}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status != 200:
...
async def definir(ctx: commands.Context, *, palabra: str):
    if ctx.interaction:
        await ctx.defer()
    url = f"https://api.dictionaryapi.dev/api/v2/entries/en/{urllib.parse.quote(palabra)}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status != 200:
...
async def steam(ctx: commands.Context, *, juego: str):
    ...
    app_id, nombre = juego_info['appid'], juego_info['name']
    ...
    embed = discord.Embed(title=nombre,
                          ...
                          color=0x2B55B5, url=f"https://store.steampowered.com/app/{app_id}")
...
async def covid(ctx: commands.Context, pais: str = "mexico"):
    if ctx.interaction:
        await ctx.defer()
    url = f"https://disease.sh/v3/covid-19/countries/{urllib.parse.quote(pais)}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status != 200:
...
async def google_search(ctx: commands.Context, *, query: str):
    ...
    for i, res in enumerate(resultados[:5]):
        ...
        enlace  = res.get('link', '#')
        embed.add_field(name=f"{i+1}. {titulo[:70]}",
                        value=f"> {snippet[:150]}\n[Leer más]({enlace})", inline=False)
    await ctx.send(embed=embed)
...
async def imagenes_search(ctx: commands.Context, *, query: str):
    ...
    for i, img in enumerate(imagenes[:3]):
        embed.add_field(name=f"> Imagen {i+1}", value=f"[Ver imagen]({img.get('imageUrl', '#')})", inline=True)
    await ctx.send(embed=embed)
...
async def lugares_search(ctx: commands.Context, *, lugar: str):
    ...
    headers = {"User-Agent": "MistiBot/1.0"}
    try:
        async with aiohttp.ClientSession() as session:
...
async def noticias(ctx: commands.Context, *, query: str = None):
    ...
    async with ctx.typing():
        if query:
            query_encoded = urllib.parse.quote(query)
            feeds = [f"https://news.google.com/rss/search?q={query_encoded}&hl=es&gl=ES&ceid=ES:es",
                     "https://feeds.bbci.co.uk/mundo/rss.xml"]
        else:
            feeds = ["https://news.google.com/rss?hl=es&gl=ES&ceid=ES:es",
                     "https://feeds.bbci.co.uk/mundo/rss.xml",
        ...
        try:
            ...
            await ctx.send("> Ocurrió un error al procesar las noticias.")
...
async def doctor(ctx: commands.Context):
    ...
    if not me:
        embed = discord.Embed(title="Doctor - Diagnóstico",
    ...
    def formatear_permisos(lista_permisos):
        ...
        return texto
    embed = discord.Embed(title="Diagnóstico de Salud del Bot",
                          description="Estado de permisos del bot.", color=0x2B55B5)
    ...
    embed.set_footer(text=f"Misti Doctor • Latencia: {round(bot.latency * 1000)}ms",
                     icon_url=ctx.author.display_avatar.url)
    await ctx.send(embed=embed)

# =========================================================
# COMANDOS — IPOD / LOGRO / PRIMER MENSAJE
# =========================================================
...
async def _animal_embed(ctx: commands.Context, datos: dict, query_unsplash: str, fallback_url: str):
    await ctx.defer()
...
class ModalAnonimo(discord.ui.Modal, title="Mensaje Anónimo"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        guardar_anonimos()
        embed = discord.Embed(description=str(self.contenido), color=0x2B55B5)
        embed.set_footer(text=f"#{numero:03d}")
        await self.canal_destino.send(embed=embed)
        await interaction.response.send_message("> Tu mensaje anónimo fue enviado correctamente.", ephemeral=True)
...
@bot.hybrid_command(name="set-anonimos", description="Configura el canal de mensajes anónimos y envía el panel desde cero.")
@commands.has_permissions(administrator=True)
async def set_anonimos(ctx: commands.Context, canal: discord.TextChannel):
    ...
    embed_panel = discord.Embed(
        ...
        description="Estos son los mensajes anónimos, mensajes de cualquier usuario serán enviados aquí.\n\n> Interactúa con el botón de abajo para mandar tu mensaje anónimo.",
        color=0x2B55B5
    )
    vista = VistaPanelAnonimo(canal)
    await canal.send(embed=embed_panel, view=vista)
    await ctx.followup.send(f"> Panel de mensajes anónimos configurado en {canal.mention}.", ephemeral=True)

@bot.hybrid_command(name="panel-anonimos", description="(ADMIN) Vuelve a enviar el panel de mensajes anónimos sin reiniciar la cuenta.")
@commands.has_permissions(administrator=True)
async def panel_anonimos(ctx: commands.Context):
    await ctx.defer(ephemeral=True)
    gid = str(ctx.guild.id)
    if gid not in anon_config:
        await ctx.followup.send("> Primero configura el canal con `/set-anonimos`.", ephemeral=True)
        return
    canal_id = anon_config[gid]["canal_id"]
    canal    = ctx.guild.get_channel(canal_id)
    if not canal:
        await ctx.followup.send("> No se encontró el canal configurado. Usa `/set-anonimos` para reconfigurarlo.", ephemeral=True)
        return
    ...
    vista = VistaPanelAnonimo(canal)
    await canal.send(embed=embed_panel, view=vista)
    await ctx.followup.send(f"> Panel reenviado en {canal.mention}. La cuenta sigue desde **#{total_actual:03d}**.", ephemeral=True)

# =========================================================
# IA — MEMORIA / GENERACION DE IMAGENES / ASK
# =========================================================
...
PALABRAS_IMAGEN = ["imagen", "foto", "dibujo", "genera", "wallpaper", "crea", "hazme", "dibujame",
                   "pintame", "ilustra", "generame", "muestrame"]
...
async def on_member_join(member):
    ...
    if autor_n:
        embed.set_author(name=parse_text(autor_n, member),
                         icon_url=parse_text(autor_i or "", member) or None)
    if cfg.get("imagen"):
        embed.set_image(url=parse_text(cfg["imagen"], member))
    footer_t, footer_i = cfg.get("footer", (None, None))
    if footer_t:
        embed.set_footer(text=parse_text(footer_t, member),
                         icon_url=parse_text(footer_i or "", member) or None)
    await canal.send(embed=embed)
...
async def on_message(message):
    ...
    if bot.user in message.mentions:
        ...
        # Procesar comandos igualmente y salir
